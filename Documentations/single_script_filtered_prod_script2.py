...
def get_individual_stats():
    ...
    session_docs = chat_collection.find(session_docs_query, {'_id': 1, 'created_at': 1})
    ...
    for session in session_docs:
        ...
        bu_name = chat_collection.find_one({'_id':session['_id']}).get('bu_name', 'N/A')
        ...
        while current_date <= end_date:
            ...
            for document in result:
                ...
                for chat in chat_obj:
                    ...
                    activity_dict["BU"] = document.get('bu_name', 'N/A')
...
def merge_csv_to_excel_and_delete():
    try:
        ...
        current_date = datetime.now().strftime("%m-%d-%Y")
        filename = f'Usage Report PROD {current_date}.xlsx'
...
if __name__=="__main__":
    ...
    data2 = get_individual_stats()
    create_csv('Session Stats',data2)
    
    data3 = get_activity_image_data()
    create_csv('Image Logs', data3)
