...
# To parse additional document for above BU use list it here else keep it blank
specific_documents_to_parse=[]
#specific_documents_to_parse=["Test_Doc.pdf"]
...
async def process_pdf(s3_bucket_name, KnowledgeBaseName, s3_client):
    ...
    if specific_documents_to_parse!=[]:
        pdf_file_names = specific_documents_to_parse
        print("Parsing only specific documents")
    else:
...
try:
    ...
    asyncio.run(save_images(s3_client, s3_bucket_name, KnowledgeBaseName, specific_documents_to_parse))
